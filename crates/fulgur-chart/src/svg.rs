...
fn write_prim(
    ...
    match prim {
        ...
        Prim::StyledText(text) => {
            let xv = fmt_num(text.x);
            let yv = fmt_num(text.y);
            let size = fmt_num(text.size);
            let anchor = match text.anchor {
                Anchor::Start => "start",
                Anchor::Middle => "middle",
                Anchor::End => "end",
            };
            let hex = color_hex(&text.fill);
            let op = opacity_attr("fill-opacity", text.fill.a);
            let escaped = xml_escape(&text.content);
            let fam = xml_escape_attr(text.font_family.as_deref().unwrap_or(font_family));
            let weight = text
                .font_weight
                .as_deref()
