...
- For `radialGauge`, `datasets[0].data` holds a single value drawn as a fill-to-value arc on a track ring. Configure with `options.domain` / `trackColor` / `centerPercentage` / `roundedCorners`. The center value text falls back to the rounded value (JS `centerArea.text` is not executed).
- `options.indexAxis`
