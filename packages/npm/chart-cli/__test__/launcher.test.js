...
describe('detectPlatformKey', () => {
  ...
  it('detects linux arm64 glibc', () => {
    assert.strictEqual(detectPlatformKey('linux', 'arm64', () => false), 'linux-arm64');
  });

  it('rejects linux arm64 musl (no musl build)', () => {
    assert.strictEqual(detectPlatformKey('linux', 'arm64', () => true), null);
  });
...
describe('launcher errors', () => {
  it('exits 1 when platform package is missing', () => {
    ...
    const fs = require('fs');
    const os = require('os');
    const tmpdir = fs.mkdtempSync(path.join(os.tmpdir(), 'chart-cli-test-'));
    try {
      const isolatedLauncher = path.join(tmpdir, 'fulgur-chart');
      fs.copyFileSync(LAUNCHER, isolatedLauncher);
      fs.chmodSync(isolatedLauncher, 0o755);
