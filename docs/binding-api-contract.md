...
|---|---|---|---|---|---|
| Input/Parse Error | 1 | 不正 JSON、パース失敗、未知 DSL、寸法制限超過 | `FulgurParseError(ValueError)` | `FulgurParseError` | `FulgurChart::ParseError < StandardError` |
| Strict Violation | 2 | ストリクトモードでの未知キー | `FulgurStrictError(FulgurParseError)` | `FulgurStrictError < FulgurParseError` | `FulgurChart::StrictError < FulgurChart::ParseError` |
| Render/IO Error | 3 | ラスタ変換失敗、IO エラー | `FulgurRenderError(RuntimeError)` | `FulgurRenderError` | `FulgurChart::RenderError < StandardError` |

> Node.js / WASM: native 層は `code` discriminant（`'PARSE_ERROR'` / `'STRICT_ERROR'` / `'RENDER_ERROR'`）を返し、JS wrapper がその `code` を上記の階層クラスへマップする。
