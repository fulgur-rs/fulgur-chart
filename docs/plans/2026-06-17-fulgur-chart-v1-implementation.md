...
**Files:**
- Create: `crates/fulgur-chart/assets/fonts/NotoSansJP-Regular.otf`（ダウンロード）
- Create: `crates/fulgur-chart/assets/fonts/LICENSE-NotoSansJP.txt`（SIL OFL）
- Modify: `crates/fulgur-chart/src/lib.rs`

**Step 1: フォントを取得**

Noto Sans JP Regular（**static**、可変フォントではない）と OFL ライセンスを `crates/fulgur-chart/assets/fonts/` に置く（クレート内に同梱し crates.io 公開時も自己完結させる）。
...
Run: `ls -la crates/fulgur-chart/assets/fonts/`
Expected: `NotoSansJP-Regular.otf` が存在（数 MB）。`font.rs` の `include_bytes!` のパスを実ファイル名に合わせる。
