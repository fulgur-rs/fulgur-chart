...
Noto Sans JP Regular（static TTF）と OFL ライセンスを `assets/fonts/` に置く。
入手元は Google Fonts / notofonts。取得後にサイズを確認:

Run: `ls -la assets/fonts/`
Expected: `NotoSansJP-Regular.ttf` が存在（数 MB）。
...
fn parses_minimal_bar_spec() {
    ...
    // 色未指定 → パレット先頭(#36A2EB)
    assert_eq!(
        (spec.series[0].fill.r, spec.series[0].fill.g, spec.series[0].fill.b),
        (54, 162, 235)
    );
}
...
struct RawDataset {
    ...
    #[serde(rename = "borderColor")] border_color: Option<String>,
    #[serde(rename = "borderWidth")] border_width: Option<f64>,
    #[serde(default)] fill: bool,
...
pub fn parse(json: &str, strict: bool) -> Result<ChartSpec, String> {
    ...
    let series = raw.data.datasets.iter().enumerate().map(|(i, ds)| {
        let base = palette_color(i);
        let fill = ds.background_color.as_deref().and_then(parse_color).unwrap_or(base);
        let stroke = ds.border_color.as_deref().and_then(parse_color).unwrap_or(base);
        Series {
            name: ds.label.clone(),
            values: ds.data.clone(),
            fill,
            stroke,
...
> 注: usvg 0.45 の `Options.fontdb` の正確な型（`Arc<Database>` 等）は API を確認して合わせる。
